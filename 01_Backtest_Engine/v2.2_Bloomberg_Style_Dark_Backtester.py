...
class PortfolioBacktester:
    ...
    def run_backtest(self, start_date, end_date):
        ...
        plt.plot(normalized_comparison.index, normalized_comparison['Portfolio'], label='Portfolio', linewidth=3, color='cyan', alpha=0.3)  # 원본 데이터 (투명하게)
        plt.plot(pd.to_datetime(normalized_comparison.index[x_new.astype(int)]), y_portfolio_smooth, label='Portfolio (Smoothed)', linewidth=3, color='cyan')  # 보간된 데이터

        # S&P 500 선
        spl_sp500 = make_interp_spline(x, normalized_comparison['S&P 500'], k=3)
        y_sp500_smooth = spl_sp500(x_new)
        plt.plot(normalized_comparison.index, normalized_comparison['S&P 500'], label='S&P 500', linewidth=2, color='magenta', linestyle='--', alpha=0.3)  # 원본 데이터 (투명하게)
        plt.plot(pd.to_datetime(normalized_comparison.index[x_new.astype(int)]), y_sp500_smooth, label='S&P 500 (Smoothed)', linewidth=2, color='magenta')  # 보간된 데이터

        # KOSPI 선
        spl_kospi = make_interp_spline(x, normalized_comparison['KOSPI'], k=3)
        y_kospi_smooth = spl_kospi(x_new)
        plt.plot(normalized_comparison.index, normalized_comparison['KOSPI'], label='KOSPI', linewidth=2, color='yellow', linestyle='-.', alpha=0.3)  # 원본 데이터 (투명하게)
        plt.plot(pd.to_datetime(normalized_comparison.index[x_new.astype(int)]), y_kospi_smooth, label='KOSPI (Smoothed)', linewidth=2, color='yellow')  # 보간된 데이터
